...
export function Board({ initial, onDrop }: BoardProps) {
    ...
    return combine(
      monitorForElements({
        ...
        onDrop({ source, location }) {
          ...
          // Check if we're dropping at the beginning of a column
          const isDroppingAtBeginning = location.current.dropTargets.length > 0 && 
            location.current.dropTargets[0].data[cardDropTargetKey] && 
            extractClosestEdge(location.current.dropTargets[0].data) === 'top';

          // Don't allow dropping at the beginning of a column
          if (isDroppingAtBeginning) {
            return;
          }

          // Call the onDrop callback if provided
          if (onDrop && isColumnData(dropTargetData)) {
            onDrop({ data: dragging }, { data: dropTargetData }, 0);
          }
          ...
          if (isColumnData(dropTargetData)) {
            ...
            // insert into destination list at the bottom
            const destinationCards = Array.from(destination.cards);
  ...
  return (
    <div className={`flex h-full flex-col ${settings.isBoardMoreObvious ? 'px-32 py-20' : ''}`}>
      <div
        className={`flex h-full flex-row gap-3 overflow-x-auto [scrollbar-color:theme(colors.gray.400)_theme(colors.sky.500)] [scrollbar-width:thin] ${settings.isBoardMoreObvious ? 'rounded border-2 border-dashed' : ''}`}
        ref={scrollableRef}
      >
